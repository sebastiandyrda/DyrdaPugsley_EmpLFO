...
N = 3; % number of LFO types: C-corp, Passthrough, Other

raw_file = '../data/restricted/disclosed/req10739_20230720/disclosure_output_post.xlsx';
outxls_file = '../data/decomp/figure_cf_series2.xls';
...
% initial condition
initial_shares(1:end-1) = readmatrix(raw_file,'Sheet','initial shares_T13T26','Range','B2:C2');
initial_shares(end) = 1-sum(initial_shares(1:end-1));
...
for t=2:T
    ...
    P_t(:,:,t) = data_year_withz([1,3:4],[1,3:4]);
    exit_rates_t(:,t) = data_year_withz([1,3:4],2)/100;
    entrant_shares_t(:,t) = data_year_withz(2,[1,3:4])';
end
...
shares_actual2_t = shares_actual_t;

shares_cf_nostartupdef_t = shares_actual_t;
shares_cf_nostartupdef2_t = shares_actual_t;
...
for t=2:T
    ...
    shares_actual2_t(:,t) = P_t(:,:,t)'*diag((1-exit_rates_t(:,t))/(1-shares_actual2_t(:,t-1)'*exit_rates_t(:,t)))*shares_actual2_t(:,t-1)*(1-startup_rate_t(t)) + entrant_shares_t(:,t)*startup_rate_t(t);
...
for t=2:T
    ...
    shares_cf_nostartupdef2_t(:,t) = P_t(:,:,t)'*diag((1-mean(exit_rates_t(:,2:3),2))/(1-mean([shares_actual2_t(:,1)'*exit_rates_t(:,2),shares_actual2_t(:,2)'*exit_rates_t(:,3)])))*shares_cf_nostartupdef2_t(:,t-1)*(1-mean(startup_rate_t(2:3))) + entrant_shares_t(:,t)*mean(startup_rate_t(2:3));
...
for t=2:T
    flow_entrant_actual(t) = sum(entrant_shares_t(2,t))/sum(entrant_shares_t(1:2,t));
    flow_reorg_ctop_actual(t) = P_t(1,2,t)/sum(P_t(1,1:2,t));
    flow_reorg_ptoc_actual(t) = P_t(2,1,t)/sum(P_t(1,1:2,t));
    ...
    if t>=range_index_post_spike(1)
        flow_entrant_cf_permanent_both(t) = mean(entrant_shares_t(2,range_index_post_spike)./sum(entrant_shares_t(1:2,range_index_post_spike),1),2);
        flow_reorg_ctop_cf_permanent_both(t) = mean(P_t(1,2,range_index_post_spike)./sum(P_t(1,1:2,range_index_post_spike),2),3);
        flow_reorg_ptoc_cf_permanent_both(t) = mean(P_t(2,1,range_index_post_spike)./sum(P_t(1,1:2,range_index_post_spike),2),3);
    end
end
...
getnozptshares = @(shares_) shares_(2,:)./sum(shares_(1:end-1,:),1);
...
outtab = table( ...,
    ...
    getnozptshares(shares_cf_holdall_t)', ...
    getnozptshares(shares_cf_holdboth_t)', ...
    getnozptshares(shares_cf_holdentry_t)', ...
    getnozptshares(shares_cf_holdreorg_t)', ...
    getnozptshares(shares_cf_permanent_both_t)',...
    flow_entrant_actual', ...
    flow_reorg_ctop_actual', ...
    flow_reorg_ptoc_actual', ...
    flow_entrant_cf_permanent_both', ...
    flow_reorg_ctop_cf_permanent_both', ...
    flow_reorg_ptoc_cf_permanent_both',...
    getnozptshares(shares_target_t)', ...
    getnozptshares(shares_cf_target_holdall_t)',...
    getnozptshares(shares_cf_target_holdboth_t)', ...
    getnozptshares(shares_cf_target_holdentry_t)', ...
    getnozptshares(shares_cf_target_holdreorg_t)', ...
    getnozptshares(shares_cf_nostartupdef_t)', ...
    exit_rates_t(2,:)', ...
    exit_rates_t(1,:)', ...
    startup_rate_t', ...
    startup_rate_const, ...
    shares_actual_t(1,:)', ... c share
    shares_actual_t(2,:)', ... pt share
    1-shares_actual_t(1,:)'-shares_actual_t(2,:)', ... z share
    entrant_shares_t(1,:)', ... c share entrants
    entrant_shares_t(2,:)', ... pt share entrants
    1-entrant_shares_t(1,:)'-entrant_shares_t(2,:)' ... z share entrants
    );
